	...
	<div class="row">
		<div class="col">
			<div class="card estilo-card cor-flutuante text-center">
				<h2 class="float-start titulo-card mt-2 fw-bold">Editar Receitas:</h2>
				<div class="card-body centralizaB">
					...
					<form action="fintech" method="post">
						...
						<input type="hidden" value="${recebimento.codigoRecebimento}" name="codigo">
						<input type="hidden" value="${recebimento.instituicao.numeroInstituicao}" name="instituicao">
						<input type="hidden" value="${recebimento.conta.numeroConta}" name="conta">
						<div class="form-group mt-1 fw-bold">
							<label for="id-valor">Valor</label> <input type="text"
								name="valor" id="id-valor" class="form-control" value="${recebimento.valor}">
						</div>
						<div class="form-group mt-1 fw-bold">
							<label for="id-descricao">Descrição</label> <input type="text"
								name="descricao" id="id-descricao" class="form-control" value="${recebimento.descricao}">
						</div>
						<div class="form-group mt-1 fw-bold">
							<label for="id-data">Data</label> <input type="text" name="data"
								id="id-data" class="form-control" value='<fmt:formatDate value="${recebimento.data.time }" pattern="dd/MM/yyyy"/>'>
						</div>
						<fieldset disabled>
						<div class="form-group mt-1 fw-bold">
							<label for="id-instituicao">Instituição financeira</label> <input
								type="text" name="" id="id-instituicao"
								class="form-control" value="${recebimento.instituicao.numeroInstituicao}">
						</div>
						<div class="form-group mt-1 fw-bold">
							<label for="id-conta">Conta corrente</label> <input type="text"
								name="" id="id-conta" class="form-control" value="${recebimento.conta.numeroConta}">
						</div>
						</fieldset>
						<input type="submit" value="Salvar" class="btn btn-primary mt-2">
						<a href="fintech?acao=listarReceitas" class="btn btn-danger mt-2">Voltar</a> 
					</form>
				</div>
		  </div>
	 </main>
